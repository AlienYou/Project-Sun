...
- `LensAnchor` 只承载倍镜 Render Texture 的显示平面与口径；移动它不能改变 ADS 姿态、弹道或配件挂载。
...
当数值输入不便于判断空间关系时，在 **Scene 直接编辑** 中选择“移动模型”、“旋转模型”或“移动 ADS 瞄准基准”。启动实时预览后，Scene 视图会显示对应彩色手柄；一次鼠标拖拽会合并为一次 Undo，并沿用工作台的延迟保存机制。`LensAnchor` 的专用实时校准控件属于当前真实透镜阶段的后续工具交付；在该控件完成前，不要用 ADS 瞄准基准替代镜片调整。
...
`Model` 与 ADS 瞄准基准的改动会立即反映到实时预览；停止编辑约 0.45 秒后，工作台才保存附件预制体，也可点击“立即保存”。这样连续拖动或键入数值不会反复触发资源保存和导入。一次模型与锚点编辑会合并为一个 Unity Undo 操作，`Ctrl+Z` / `Ctrl+Y` 后会同步刷新预览并保存结果。
