...
状态：现行。本文是动态附件、ADS Workbench、校准根与倍率镜生产流程的权威规范。
...
在 WeaponLab 按 `F7` 启动或中止倍率镜压力测试。该入口只在 Editor 或 Development Build 中启用独占测试输入，正式非开发构建不能打开；测试期间玩家移动、视角和人工武器输入会暂时归零，离散切枪仍调用 `WeaponInventoryController` 的正式公开状态机，不复制切枪规则。

一次测试会保存启动前的 Unity 画质档位，并对每个现有档位依次执行：快速开镜/退镜、稳定 ADS 连续开火、切至副武器、再切回主武器。每次稳定 ADS 会验收 `ScopeSightRenderer` 为 `READY`、项目镜片 Shader 未进入后备、Render Texture 已创建，并确认镜外 Pass 与当前画质对应为 `DIM ONLY`、`BLUR 4 TAP` 或 `BLUR 8 TAP`。HUD 显示当前画质、阶段和进度；完整失败原因写入 Console。测试完成、再次按 `F7`、按 `F6` 或组件停用时都会释放测试输入并恢复原画质。

自动结果只证明资源生命周期和渲染链在重复状态切换中保持有效，不能判断镜框遮挡、动态口径跟随观感、锯齿、GPU 帧耗或正式关卡光照。显示 `PASS` 后仍需人工观察全过程，并在目标显卡使用 GPU Profiler 记录倍率相机、口径 Mask 与镜外模糊 Pass 的耗时和显存。
