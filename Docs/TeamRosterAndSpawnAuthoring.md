...
- `RoundManager.TryGetNextLocalSpectatorTarget` 仅在战斗回合、本地玩家已死亡且己方仍有存活成员时返回目标；它只发布规则允许的目标，不负责移动或切换摄像机。
- 当前阶段只显示“观战目标已准备”的等待信息。实际观战摄像机和切换输入属于下一步，不能由 HUD 临时接管玩家相机来绕过正式状态接口。
